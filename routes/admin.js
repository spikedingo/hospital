...
//对象新增
    router.post('/manage/:defaultUrl/addOne',function(req,res,next){
        ...
        console.log('adding', currentPage, targetObj)

        if(adminFunc.checkAdminPower(req, + '_add')){
            if(targetObj == AdminUser){
            ...
            }else if(targetObj == Department){
                DbOpt.addOne(targetObj,req, res);
            }else if (targetObj == Doctor){
                DbOpt.addOne(targetObj,req, res);
            }else if(targetObj == ContentTags){
...
//文章置顶
    ...
    //党建文章置顶
        ...
        //------------------------------------------通知管理面开始
        //通知列表页面
            router.get('/manage/noticesList', function(req, res, next) {

                adminFunc.renderToManagePage(req, res,'manage/noticesList',settings.NOTICESLIST);
        ...
        //通知添加页面(默认)
            router.get('/manage/notices/add/:key', function(req, res, next) {
                ...
                if(contentType == "plug"){
                    ...
                    targetPath = 'manage/addNotices';
                }

                res.render(targetPath, adminFunc.setPageInfo(req,res,settings.NOTICESLIST));
        ...
        //通知编辑页面
            router.get('/manage/notices/edit/:type/:content', function(req, res, next) {
                var contentType = req.params.type;
                var targetPath;
        ...
        //通知置顶
            router.get('/manage/noticesList/topContent', function(req, res, next) {
                var params = url.parse(req.url,true);
                var contentId = params.query.uid;
                var isTop = Number(params.query.isTop);
                if(shortid.isValid(contentId)){
                    if(adminFunc.checkAdminPower(req,settings.NOTICESLIST[0] + '_top')){
                        Content.update({_id : contentId}, {'isTop' : isTop}, function (err,result) {
                            if(err) throw  err;
                            res.end("success");
                        })
...
//添加新类别
    function addOneCategory(req,res){
        ...
        console.log(req.body, newObj, 'adding category')
...
//科室列表页面
    ...
    router.get('/manage/departmentList', function(req, res, next) {

        adminFunc.renderToManagePage(req, res,'manage/departmentList',settings.DEPARTMENTLIST);
...
//添加科室页面
    ...
    router.get('/manage/department/add/:key', function(req, res, next) {
        ...
        targetPath = 'manage/addDepartment';

        res.render(targetPath, adminFunc.setPageInfo(req,res,settings.DEPARTMENTLIST));
...
//编辑科室页面
    ...
    router.get('/manage/department/edit/:department', function(req, res, next) {
        var contentType = req.params.type;
        var targetPath;
...
//添加医生页面
    ...
    router.get('/manage/doctor/add/:key', function(req, res, next) {
        var contentType = req.params.key;
        var targetPath;

        targetPath = 'manage/addDoctor';

        res.render(targetPath, adminFunc.setPageInfo(req,res,settings.DOCTORLIST));
...
//编辑医生页面
    ...
    router.get('/manage/doctor/edit/:doctor', function(req, res, next) {
        var contentType = req.params.type;
        var targetPath;
