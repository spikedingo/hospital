...
doraApp.controller('contentList',['$scope','$http',function($scope,$http){
    ...
    $scope.topContent = function(id,m){
        angularHttpGet($http,"/admin/manage/contentUpdate/topContent?uid="+id+"&isTop="+m,function(){
            initPagination($scope,$http);
        });
    };
}]);
...
doraApp.controller('departmentList',['$scope','$http',function($scope,$http){
    $scope.formData = {};
    //获取文档列表信息
    initPagination($scope,$http);
    //删除文档
    initDelOption($scope,$http,'您确认要删除选中的科室吗？');
}]);
...
doraApp.controller('doctorList',['$scope','$http',function($scope,$http){
    $scope.formData = {};
    //获取文档列表信息
    initPagination($scope,$http);
    //删除文档
    initDelOption($scope,$http,'您确认要删除选中的医生吗？');
}]);
